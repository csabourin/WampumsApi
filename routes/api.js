...
router.post(
        "/verify-email",
        [check("verification_token").notEmpty(), validateRequest],
        authController.verifyEmail,
);

router.post(
        "/request-reset",
        [check("email").isEmail().normalizeEmail(), validateRequest],
...
router.post(
        "/approve-user",
        [check("user_id").notEmpty(), validateRequest, roleMiddleware(["admin"])],
...
router.get(
        "/participants-with-users",
        participantController.getParticipantsWithUsers,
);
router.post(
        "/link-participant-to-organization",
        [check("participant_id").notEmpty(), validateRequest],
...
router.post(
        "/link-user-participants",
        [check("participant_ids").isArray(), validateRequest],
...
router.get(
        "/guardians-for-participant",
        guardianController.getGuardiansForParticipant,
);
router.post(
        "/save-parent",
        [check("participant_id").notEmpty(), validateRequest],
...
router.post(
        "/add-group",
        [check("group_name").notEmpty().trim(), validateRequest],
        groupController.addGroup,
);
router.post(
        "/remove-group",
        [check("group_id").notEmpty(), validateRequest],
...
router.get(
        ...
        formController.getOrganizationFormFormats,
);
...
router.post(
        "/approve-badge",
        [check("badge_id").notEmpty(), validateRequest],
...
router.post(
        ...
        [check("date").isISO8601(), validateRequest],
...
router.get(
        ...
        reportController.getMediaAuthorizationReport,
);
router.get(
        "/missing-documents-report",
        reportController.getMissingDocumentsReport,
);
...
router.post(
        "/create-organization",
        [check("name").notEmpty().trim(), validateRequest],
        organizationController.createOrganization,
);
router.post(
        "/switch-organization",
        [check("organization_id").notEmpty(), validateRequest],
