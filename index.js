...
const allowedOriginPatterns = [
  /^https:\/\/.*\.test/,
  /^https:wampums.*\.test$/, // All worf.replit.dev subdomains
...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    const isAllowed = allowedOriginPatterns.some((pattern) => {
      ...
      console.log(`Testing ${pattern} against ${origin}: ${matches}`);
      return matches;
    });
...
process.on("unhandledRejection", (reason, promise) => {
  ...
  // Handle different types of reasons appropriately
  if (reason instanceof Error) {
    ...
    try {
      ...
      console.error("Could not stringify reason:", reason);
    }
  } else {
    console.error(reason);
  }
